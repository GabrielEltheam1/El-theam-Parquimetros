...
export default async function handler(req, res) {
  ...
  if (path === '/parquimetros' && req.method === 'GET') {
    ...
    return json(res, data.map(p => ({
      id: p.id, direccion: p.direccion, lat: p.lat, lon: p.lon
    })));
  }
  ...
  if (path === '/baterias' && req.method === 'GET') {
    ...
    if (error) return json(res, { error: error.message }, 500);
    return json(res, data.map(b => ({
      id: b.id, numero: b.numero, marca: b.marca, fechaIngreso: b.fecha_ingreso, notas: b.notas
    })));
  }
  ...
  if (path === '/baterias' && req.method === 'POST') {
    ...
    if (numero !== 'SN') {
      const { data: existing } = await supabase.from('baterias').select('id').eq('numero', numero).single();
  ...
  if (path.match(/^\/baterias\/[^/]+$/) && req.method === 'DELETE') {
    const id = parts[1];
    await supabase.from('baterias').delete().eq('id', id);
    return json(res, { ok: true });
  }
  ...
  if (path.match(/^\/cambios\/[^/]+$/) && req.method === 'DELETE') {
    const id = parts[1];
    await supabase.from('cambios').delete().eq('id', id);
    return json(res, { ok: true });
  }
  ...
  if (path === '/cargadores' && req.method === 'GET') {
    ...
    if (error) return json(res, { error: error.message }, 500);
    // Group into cargador objects
    const map = {};
    for (let c = 1; c <= 5; c++) {
      map[c] = { numero: c, slots: [] };
      for (let s = 1; s <= 5; s++) {
        map[c].slots.push({ numero: s, bateriaId: null, desde: null });
      }
    }
  ...
  if (path === '/stats/baterias' && req.method === 'GET') {
    ...
    const result = (bats || []).map(bat => {
      ...
      const estadias = entraEvents.map(entrada => {
        ...
        return {
          parquimetroId: entrada.parquimetro_id,
          direccion: parqMap[entrada.parquimetro_id] || '?',
          fechaEntrada: entrada.fecha,
          fechaSalida: salida ? salida.fecha : null,
          dias
        };
      });
      return {
        id: bat.id, numero: bat.numero, marca: bat.marca,
        fechaIngreso: bat.fecha_ingreso, notas: bat.notas,
        totalEntradas: entraEvents.length,
        totalSalidas: saleEvents.length,
        estadias
      };
    });
    return json(res, result);
  ...
  if (path === '/stats/parquimetros' && req.method === 'GET') {
    ...
    const result = (parqs || []).map(parq => {
      ...
      for (let i = 1; i < cc.length; i++) {
        duraciones.push(Math.round((new Date(cc[i].fecha) - new Date(cc[i-1].fecha)) / 86400000));
      }
      ...
      return {
        id: parq.id, direccion: parq.direccion,
        totalCambios: cc.length,
        promDiasPorCarga: promDias,
        minDiasPorCarga: minDias,
        bateriasBajasDuracion: duraciones.filter(d => d <= 1).length
      };
    });
    return json(res, result);
  }
  ...
  if (path === '/stats/mensual' && req.method === 'GET') {
    ...
    const { data: cambios } = await supabase.from('cambios')
      .select('*').like('fecha', `${prefijo}%`).order('fecha');
    ...
    const rows = parqIds.map(pid => {
      ...
      const totalCambios = (cambios || []).filter(c => c.parquimetro_id === pid).length;
      return { id: pid, direccion: parq ? parq.direccion : '?', dias, totalCambios };
    });
